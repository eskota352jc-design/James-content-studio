...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/oauth/status") {
      if (!env.TIKTOK_TOKENS) return json({ ok: false, connected: false, error: "Secure token storage is not configured" }, 500, corsHeaders);
      const activeOpenId = await env.TIKTOK_TOKENS.get("tiktok:active_open_id");
      if (!activeOpenId) return json({ ok: true, connected: false }, 200, corsHeaders);
      try {
        const token = await getValidToken(env, activeOpenId);
        ...
        return json({ ok: false, connected: false, error: error.message }, 401, corsHeaders);
      }
    }
...
async function getValidToken(env, openId) {
  ...
  const next = {
    ...stored,
    open_id: refreshed.body.open_id || stored.open_id,
    access_token: refreshed.body.access_token,
    refresh_token: refreshed.body.refresh_token || stored.refresh_token,
    scope: refreshed.body.scope || stored.scope,
    token_type: refreshed.body.token_type || stored.token_type,
    access_token_expires_at: now + Number(refreshed.body.expires_in || 0) * 1000,
    refresh_token_expires_at: now + Number(refreshed.body.refresh_expires_in || 0) * 1000,
    refreshed_at: now,
  };
